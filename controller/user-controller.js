...
export const signupUser = async (request, response) => {
  try {
    ...
    return response.status(201).json({
      statusCode: 201,
      message: `Hello Dear ${name} , Your signup has been successfully completed !!`,
    });
  } catch (error) {
    return response
      .status(500)
...
export const userLogin = async (request, response) => {
  try {
    ...
    if (emailMatch && passwordMatch) {
      let accessToken = jwt.sign(user.toJSON(), process.env.ACCESS_SECRET_KEY, {
        //expiresIn: "15m",
      });
    ...
    return response
      .status(500)
      .json({ statusCode: 500, message: "Some Error Occured!" });
  }
};
