...
export const jwtAuthenticateToken = (request, response, next) => {
  ...
  if (token == null) {
    return response
      .status(401)
      .json({ statusCode: 401, message: "token is missing!!!" });
  }
  ...
  jwt.verify(token, process.env.ACCESS_SECRET_KEY, (error, user) => {
    ...
    if (error) {
      return response
        .status(403)
        .json({ statusCode: 403, message: "Token is invalid!!!" });
    }
    // if we do not get any error then we pass the user detail in request.user
    request.user = user;
